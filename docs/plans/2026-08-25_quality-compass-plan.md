...
| 2 | Automatic outcome envelopes from harness, router, Codex runs, and CI | External evidence required; no backend-success labels | Harness, auto-router, Codex/Hermes, CI, and passive Kanboard/Odin status implemented |
...
artifact. Kanboard/Odin consumes a separate passive status packet that cannot
close, move, retry, train from, or otherwise mutate a task by itself.
